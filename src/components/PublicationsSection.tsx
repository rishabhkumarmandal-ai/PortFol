  ...
  const publications = [
    {
      ...
      authors:
        "Rishabh Kumar Mandal, Dinesh Kumar, Ravinder Singh, Pragya Anup, Shekhar Chamoli, Pinki Singh",
      journal:
        "IEEE Conference on Advances in Computer Science, Electrical, Electronics, and Communication Technologies (CE2CT)",
      year: "2025",
      status: "Published",
    ...
    },
    {
      ...
      authors:
        "Rishabh Kumar Mandal, Bhagatah chandra Marella, Gopi Chand Vegineni, Sireesha Addankani, Ehsaan Ellahi, Arun kumar",
      journal:
        "IEEE Conference on Advances in Computer Science, Electrical, Electronics, and Communication Technologies (CE2CT)",
      year: "2025",
      status: "Published",
        ...
        <div className="max-w-6xl mx-auto">
          <div className="-mx-2 flex snap-x snap-mandatory gap-6 overflow-x-auto px-2 pb-4">
            {publications.map((pub, index) => (
              <div
                key={pub.id}
                className="min-w-[85vw] shrink-0 snap-start rounded-3xl border border-white/10 bg-white/5 p-8 backdrop-blur-md transition-all duration-500 hover:scale-[1.02] hover:border-ocean-400/50 md:min-w-[520px] lg:min-w-[560px] animate-fade-in"
                style={{ animationDelay: `${index * 0.2}s` }}
              >
                <div className="mb-4 flex items-start justify-between gap-4">
                  <div className="flex items-center">
                    <FileText className="mr-3 text-ocean-300" size={24} />
                    <span className="font-medium text-ocean-300">{pub.type}</span>
                  </div>
                  <div className={`rounded-full border px-3 py-1 text-sm font-medium ${getStatusColor(pub.status)}`}>
                    {pub.status}
                  </div>
                </div>

                <h3 className="mb-3 text-xl font-bold text-white transition-colors hover:text-ocean-300">{pub.title}</h3>
                ...
                <div className="mb-4 space-y-2">
                  ...
                  <div className="flex items-center text-white/70">
                    ...
                    <span className="text-sm">{[pub.journal, pub.year].filter(Boolean).join(' • ')}</span>
                  </div>
                </div>

                {pub.description && <p className="mb-6 leading-relaxed text-white/80">{pub.description}</p>}

                <a
                  href={pub.link}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="rounded-hover inline-flex items-center rounded-lg bg-ocean-500/20 px-4 py-2 text-sm font-medium text-ocean-300 transition-colors duration-300 hover:bg-ocean-500/30"
                >
